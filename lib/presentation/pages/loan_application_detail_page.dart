...
class LoanApplicationDetailPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          // Solo mostrar botón de cambiar estado si NO está desembolsado
          if (!isDisbursed)
            IconButton(
  ...
  Future<void> _calculateScoring(BuildContext context) async {
    ...
    try {
      // Debug: Ver qué IDs estamos enviando
      print('🔍 Calculando scoring para:');
      print('   microfinancieraId: ${application.microfinancieraId}');
      ...
      if (result['scoring'] != null) {
        print('   score: ${result['scoring']['score']}');
        print('   band: ${result['scoring']['band']}');
      }

      if (result['decision'] != null) {
        print('   result: ${result['decision']['result']}');
        print('   comments: ${result['decision']['comments']}');
      }
  ...
  Future<void> _changeStatus(BuildContext context, String newStatus) async {
    ...
    final repository = context.read<IntakeRequestRepository>();
    ...
    try {
      await repository.updateStatus(application.id, newStatus);
